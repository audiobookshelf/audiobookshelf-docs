...
A series sequence also supports non-numeric values, as long as they have no spaces. These values are not sorted in any special order, but they are placed before any books that have a numeric series sequence.
...
:::info Empty Series
Series are automatically removed from the database when no books are in the series. Some older verisons of the server would not clean up empty series correctly. To remove an empty or duplicate series: add any book to the series, save the book, and then remove the book from the series to trigger the cleanup again.
:::
