...
An example OPF file is shown below:
```
...
>
  <metadata>
    <!-- Standard OPF fields parsed by Audiobookshelf -->
    <dc:title>Example Title</dc:title>
    <dc:description>Example plain-text description for the audiobook.</dc:description>
    <dc:creator opf:role="aut" opf:file-as="Author, Example">Example Author</dc:creator>
    <dc:creator opf:role="nrt" opf:file-as="Narrator, Example">Example Narrator</dc:creator>
    ...
    <dc:identifier id="AsinId" opf:scheme="ASIN">B000000000</dc:identifier>

    <!-- Audiobookshelf-supported OPF extensions -->
    <dc:subtitle>Example Subtitle</dc:subtitle>
    <meta name="calibre:series" content="Example Series" />
    <meta name="calibre:series_index" content="1" />
    <dc:tag>sample</dc:tag>
  </metadata>
</package>
```
...
Book Description
================
Example plain-text description for the audiobook.
This file is a scanner example for Audiobookshelf docs.
```
...
Manual matching allows you to select which search result to use and select individual metadata fields to use from the online provider. You can initiate a manual match using the "Match" option in the book details page.
...
Audiobookshelf allows for bulk editing the metadata for multiple items at once. You can also Shift+Click two books to select all books in between.

Once you have selected the books to bulk edit, use the Orange Pencil icon in the upper corner.

![Enter bulk edit](/pages/bulk-editing/bulk-edit.png)

Once in this view, you can either edit individal books in a grid pattern, or use the dropdown at the top to apply to all selected books.

![Open dropdown](/pages/bulk-editing/dropdown.png)

You can either `overwrite` the selected fields with the new values, or only `append` the new values to existing fields. You can use the `Apply` button to continue editing the books before `Saving` and exiting.

![Bulk editing](/pages/bulk-editing/appending.png)
...
Audiobookshelf does not support any editing or embedding metadata for eBook or comic files.
...
There are many GUI programs which can also help with fixing an audio file. Some examples are `audacity`, `foobar2000`, `audiobookconverter`, among many others.
